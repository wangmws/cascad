  ...
  KwRealTimeQuery: function(callback) {
   ...
   KwDayQuery: function(callback) {
      ...
      if (process.env.VCAP_SERVICES) {
        ...
        ibmdb.open(connString,function (err,conn) {
            if (err) return console.log(err);
            conn.query('select sum(PAYLOAD_D_KW_TOTAL) as VALUE1, cast(PAYLOAD_D_TIMESTAMP as date) AS DATE from energymeter where cast(PAYLOAD_D_TIMESTAMP as date) <= CURRENT_DATE - 7 Days group by cast(PAYLOAD_D_TIMESTAMP as date) order by cast(PAYLOAD_D_TIMESTAMP as date) ASC ', function (err, data) {
                if (err) {
                    console.log(err);
                    return callback(err);
                } else {
